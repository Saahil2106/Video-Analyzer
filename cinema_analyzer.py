...
def estimate_color_temp(frame_bgr):
    ...
    ratio = r / (b + 1e-8)
    if ratio > 1.6:
        kelvin, label = 2800, "very warm (tungsten/candlelight)"
    elif ratio > 1.25:
        kelvin, label = 3500, "warm (golden hour / indoor)"
    elif ratio > 0.9:
        kelvin, label = 5500, "neutral (daylight)"
    elif ratio > 0.75:
        kelvin, label = 7000, "cool (overcast / shade)"
    else:
        kelvin, label = 9000, "very cool (blue hour / LED)"
    return to_json_safe({
        "kelvin": kelvin,
        "label": label,
        "r_mean": round(r, 1),
...
def detect_floor_reflection(frame_bgr):
    ...
    if detected:
        rtype = (
            "specular (glare / wet floor)" if bottom_var > 800
            else "diffuse bounce (soft floor reflection)" if color_sim > 0.7
    ...
    return to_json_safe({
        "reflection_detected": bool(detected),
...
def estimate_camera_angle(frame_bgr):
    ...
    if horizon_pct < 0.3:
        angle = "high angle (camera looking down)"
    elif horizon_pct > 0.7:
        angle = "low angle (camera looking up)"
    elif 0.45 < horizon_pct < 0.55:
        angle = "eye level"
    else:
        angle = "slight tilt / three-quarter angle"
    ...
    if center_ratio > 1.6:
        shot_type = "close-up (subject fills frame)"
    elif center_ratio > 1.2:
        shot_type = "medium shot"
    elif center_ratio < 0.8:
        shot_type = "wide shot (subject small in frame)"
    else:
...
def classify_camera_movement(flow_magnitudes):
    if not flow_magnitudes or len(flow_magnitudes) < 3:
        return to_json_safe({"movement": "unknown", "shake_score": 0.0})
    ...
    elif mean > 2.5 and shake < 0.6:
        movement = "fast pan / tracking"
    elif mean > 1.0 and shake < 0.5:
        movement = "slow pan / dolly"
    elif std < 0.3 and mean > 0.5:
        movement = "smooth zoom"
    else:
        movement = "mixed / organic movement"
...
def analyze_subject_lighting(frame_bgr, bbox: dict):
    ...
    roi = frame_bgr[int(y):int(y2), int(x):int(x2)]
    ...
    for row in range(3):
        for col in range(3):
            r0 = row * cell_h
            r1 = rh if row == 2 else min(rh, r0 + cell_h)
            c0 = col * cell_w
            c1 = rw if col == 2 else min(rw, c0 + cell_w)
            cell = roi_gray[r0:r1, c0:c1]
            mean_v = float(cell.mean()) if cell.size else 0.0
    ...
    top_mean = float(np.mean(grid_vals[0:3]))
    bottom_mean = float(np.mean(grid_vals[6:9]))
    left_mean = float(np.mean([grid_vals[0], grid_vals[3], grid_vals[6]]))
    right_mean = float(np.mean([grid_vals[2], grid_vals[5], grid_vals[8]]))
    ...
    edge_avg = float(np.mean([top_mean, bottom_mean, left_mean, right_mean]))
    backlit = bool(edge_avg > center_mean * 1.25)
    rim_lit = bool(edge_avg > center_mean * 1.1 and float(roi_gray.std()) > 30)
    ...
    if mean_roi > 210:
        ...
        exposure = "slightly bright"
    elif mean_roi < 80:
        exposure = "slightly dark"
    else:
    ...
    if grad_mag > 25:
        light_type = "hard light (sharp shadows, direct source)"
    elif grad_mag > 12:
        light_type = "semi-soft light"
    else:
        light_type = "soft / diffuse light"
...
def analyze_video_cinema(video_path: str,
    ...
    if all_subject_lighting:
        summary["subject_lighting_summary"] = {
            "dominant_light_direction_on_subjects": most_common(
                [sl["lighting"]["light_direction_on_subject"] for sl in all_subject_lighting]
            ),
            "dominant_light_type": most_common(
                [sl["lighting"]["light_type_on_subject"] for sl in all_subject_lighting]
            ),
            "dominant_exposure": most_common(
                [sl["lighting"]["exposure_quality"] for sl in all_subject_lighting]
            ),
            "backlit_frames_pct": round(
                sum(1 for sl in all_subject_lighting if sl["lighting"]["backlit"])
                / len(all_subject_lighting) * 100, 1
            ),
            "rim_light_frames_pct": round(
                sum(1 for sl in all_subject_lighting if sl["lighting"]["rim_light"])
                / len(all_subject_lighting) * 100, 1
            ),
            "bounce_light_pct": round(
                sum(1 for sl in all_subject_lighting if sl["lighting"]["bounce_light_from_below"])
                / len(all_subject_lighting) * 100, 1
            ),
            "avg_lit_ratio": safe_mean(
                [sl["lighting"]["lit_ratio"] for sl in all_subject_lighting]
            ),
        }
    ...
    for i in range(0, len(sampled), seg_size):
        ...
        seg_flows = flow_mags[int(t_start * fps): int(t_end * fps)] if flow_mags else []
        ...
        seg = {
            ...
            "reflection": any(
                c["floor_reflection"]["reflection_detected"] for c in chunk
            ),
            ...
            seg["subject_lighting"] = [
                {
                    ...
                    "light_direction_on_subject": sl["lighting"]["light_direction_on_subject"],
                    "shadow_side": sl["lighting"]["shadow_side"],
                    "light_type": sl["lighting"]["light_type_on_subject"],
                    "exposure": sl["lighting"]["exposure_quality"],
                    "backlit": sl["lighting"]["backlit"],
                    "rim_light": sl["lighting"]["rim_light"],
                    "bounce_light": sl["lighting"]["bounce_light_from_below"],
                    "lit_ratio": sl["lighting"]["lit_ratio"],
                    "highlight_coords": sl["lighting"]["highlight_coords"],
                    "shadow_coords": sl["lighting"]["shadow_coords"],
                    "color_temp_on_subject": sl["lighting"]["color_temp_on_subject"],
                    "intensity_map": sl["lighting"]["intensity_map"],
                }
